...
public abstract class JDBC3ResultSet extends CoreResultSet {
    ...
    public Date getDate(int col, Calendar cal) throws SQLException {
        ...
        switch (db.column_type(stmt.pointer, markCol(col))) {
            ...
            case SQLITE_TEXT:
                try {
                	String dateFormatStr=stmt.conn.dateStringFormat;
                	String dateStr=db.column_text(stmt.pointer, markCol(col));
                	val=dateStr;
                	if(dateFormatStr.length()>dateStr.length())
                	dateFormatStr= dateFormatStr.substring(0, dateStr.length());
                	else
                    	if(dateFormatStr.length()<dateStr.length())
                	{
                    		dateStr= dateStr.substring(0, dateFormatStr.length());
                	}
                	FastDateFormat dateFormat = FastDateFormat.getInstance(dateStr, cal.getTimeZone());

                    return new java.sql.Date(dateFormat.parse(dateStr).getTime());
                }
                catch (Exception e) {
                    SQLException error = new SQLException("Error parsing time stamp;Format:"+stmt.conn.dateStringFormat+";"+val);
                    error.initCause(e);
